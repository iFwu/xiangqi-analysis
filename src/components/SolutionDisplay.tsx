...
export function SolutionDisplay({ 
  ...
  const gameOverMessage = bestMove === 'red_wins' ? '红方胜' : bestMove === 'black_wins' ? '黑方胜' : '';
  ...
  return (
    <div className="solution-section">
      ...
      <div className="solution-controls">
        <button onClick={onPreviousMove} disabled={moveHistory.length === 0}>上一步</button>
        <button onClick={onNextMove} disabled={loading || !bestMove || isGameOver}>下一步</button>
      </div>
      <div className="solution-debug">
        {loading && <p>正在计算最佳走法...</p>}
        {error && <p style={{ color: 'red' }}>{error}</p>}
        {!isGameOver && bestMove && <p>最佳走法: {moveToChineseNotation(fenCode, bestMove)}</p>}
        {isGameOver && <p style={{ fontWeight: 'bold', color: 'green' }}>绝杀终局：{gameOverMessage}</p>}
      </div>
      ...
      <div className="current-move">
        <h3>当前步: {isGameOver ? gameOverMessage : currentMoveColor}</h3>
      </div>
    </div>
  );
}